...
export class Enemy extends Character {
    ...
    updateLocal() {
        ...
        if (this._keyboardInput.isTurboKeyEnemyPressed() && !this._turboActive) {
            this.activateTurbo();
            if (this._sendInfo)
                this.socket.send(JSON.stringify(new Sprint(PlayerType.enemy)))
        }
        ...
        if (this._keyboardInput.isMovingKeyPressedEnemy() && !this._movement.isMoving()) {
            ...
            if (this._scene.isWalkable(this._target)) {
                ...
                if (this._sendInfo)
                    this.socket.send(JSON.stringify(new Movement(auxDirection, PlayerType.enemy)))
            }
